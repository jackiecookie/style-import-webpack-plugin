...
const StyleImportEndBlock = require("./StyleImportEndBlock");
const { getStyleRequest, getVarValue, findvueStyleLoad } = require("./util");
...
class InjectCssWebpackPlugin {
  ...
  init({compilation, normalModuleFactory,isProduction}) {
    ...
    compilation.dependencyTemplates.set(
      StyleImportEndBlock,
      new StyleImportEndBlock.Template(isProduction)
    );
  }
  ...
  apply(compiler) {
    ...
    compiler.hooks.compilation.tap(
      ...
      (compilation, { normalModuleFactory }) => {
        self.init({compilation, normalModuleFactory,isProduction});
        compilation.hooks.succeedModule.tap(self.name, module => {
          ...
          if (ssr) {
            module.addDependency(
              new StyleImportEndBlock(self.endPostion,module)
            );
          }
        });
        if (ssr) {
          normalModuleFactory.hooks.afterResolve.tap(self.name, data => {
            if (self.rawRequest.has(data.rawRequest)) {
              ...
              if (styleLoader.length > 0) {
                styleLoader[0].options = styleLoader[0].options||{};
